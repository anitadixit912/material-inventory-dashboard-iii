...
class AgentExecutor(A2AAgentExecutor):
    ...
    async def execute(self, context: RequestContext, event_queue: EventQueue) -> None:
        ...
        try:
            # Load stock-specific tools (always available)
            tools = list(STOCK_TOOLS)
            logger.info("Loaded %d stock tool(s): %s", len(tools), [t.name for t in tools])

            # Also load MCP tools with graceful fallback
            try:
                mcp_tools = await get_mcp_tools(use_cache=True)
                if mcp_tools:
                    tools = tools + list(mcp_tools)
                    logger.info("Added %d MCP tool(s)", len(mcp_tools))
            except Exception:
                logger.debug("No MCP tools available - continuing with stock tools only")
