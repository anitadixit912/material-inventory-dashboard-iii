...
async def call_mcp_tool_with_retry(agw_client: Any, mcp_tool: Any, **kwargs: Any) -> str:
    ...
    """
    # Log tool name and argument keys only — never log argument values as they
    # may contain business-sensitive or personal data (e.g. user IDs, order numbers).
    logger.info(f"call_mcp_tool_with_retry START: tool={mcp_tool.name}, arg_keys={list(kwargs.keys())}")
